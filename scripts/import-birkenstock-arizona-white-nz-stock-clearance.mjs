...
const REPORT_PATH =
  "/Users/mrburns_mac/Documents/Claude/Projects/MUSE/medusa-imports/birkenstock-arizona-white-nz-stock-clearance-report.json"
...
// Reference only - never written to. Standard Delivery "Birkenstock Arizona - White"
// (source title matches exactly: "Birks Arizona - White"). Used for the standard EU
// size run, weight, and category - the material/footbed details below reflect what
// the customer specified for this particular pair (PVC/faux leather, regular footbed).
const REFERENCE_PRODUCT_ID = "prod_01KVMXFB9FVS97QY34T5JG5RKK"

const TITLE = "Birkenstock Arizona - White"
const HANDLE = "birkenstock-arizona-white-nz-stock-clearance"
const EXTERNAL_ID = "NZSTOCK-CLEARANCE-BIRKENSTOCK-ARIZONA-WHITE"
const PRICE = 100
...
const DESCRIPTION = `Birkenstock Arizona - White brings Birkenstock's recognisable Arizona silhouette in a white colourway.

Two-strap sandal with adjustable buckles, a PVC (faux leather) upper, Birkenstock's contoured cork-latex footbed (regular footbed, not the soft footbed version) and a lightweight EVA outsole.
...
const tagIds = [IDS.brandTag, IDS.modelTag, IDS.whiteTag, IDS.saleTag, clearanceTag.id]
...
const payload = {
  ...
  variants: SIZES.map((size) => ({
    title: size,
    sku: `MUSE-NZ-BIRK-ARIZONA-WHITE-CLR-${size}`.replace(/[^A-Z0-9]/gi, "").toUpperCase(),
  ...
  metadata: {
    ...
    colour_source: "customer-provided description",
    ...
    seo_title: "Birkenstock Arizona - White | Clearance NZ Stock | MUSE",
    meta_description: "Clearance: Birkenstock Arizona in White, PVC upper, regular (non-soft) footbed, NZ Stock size 37 EU. $100, final sale, no refunds.",
  },
}
