...
const BASE_DIR = "/Users/mrburns_mac/Documents/Claude/Projects/MUSE/medusa-imports/yupoo-category-819225"
...
const SOURCE_CATEGORY = "https://yolo88.x.yupoo.com/categories/819225?isSubCate=true"

const SIZES = ["36", "37", "37.5", "38", "39", "39.5", "40", "40.5", "41.5", "42", "42.5", "43.5", "44", "44.5", "45", "46", "46.5", "47"]
const PRICE = 160
...
for (const album of raw) {
  ...
  if (info.confidence === "needs review") {
    report.needs_review.push({ code, source_title: album.source_title, reason: "colourway not verified; product title uses style code" })
  }
  ...
  const payload = {
    ...
    discountable: true,
    weight: 400,
    external_id: `YUP819225-${code}`,
    thumbnail: imageUrls[0],
    images: imageUrls.map((url) => ({ url })),
    options: [{ title: "Size", values: SIZES }],
    variants: SIZES.map((size) => ({
      title: size,
      sku: `MUSE-GELNYC-YUP819225-${code}-${size}`.replace(/[^A-Z0-9]/gi, "").toUpperCase(),
