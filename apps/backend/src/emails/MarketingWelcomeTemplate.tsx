...
export const MarketingWelcomeTemplate = (props: WelcomeEmailProps) => {
  ...
  if (plain) return <Html><Head /><Preview>{props.previewText}</Preview><Body style={{ backgroundColor: colors.white, fontFamily: FONT_STACK, fontSize: 14, lineHeight: "22px", color: "#222222" }}><Container style={{ maxWidth: 560, padding: "32px 20px" }}><Text style={{ margin: "0 0 16px" }}>Hi {props.firstName || "there"},</Text><Text style={{ margin: "0 0 16px" }}>Did your welcome code come through ok? It's still active if you're still deciding on a size or which piece to get. Reply here if you want a hand picking something.</Text><Text style={{ margin: "0 0 16px" }}>Code: {props.code}<br />Expires: {props.expiresAt}</Text><Text style={{ margin: "0 0 4px" }}>Sarah</Text><Text style={{ margin: "0 0 24px" }}>MUSE NZ</Text><Text style={{ color: colors.muted, fontSize: 11 }}><a href={props.unsubscribeUrl} style={{ color: colors.muted }}>Unsubscribe</a></Text></Container></Body></Html>
