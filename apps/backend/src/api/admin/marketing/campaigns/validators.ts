import { z } from "zod"
export const CampaignBlockSchema = z.object({ type: z.enum(["hero","text","product_grid","category_row","review","offer","trust","divider","html"]) }).passthrough()
export const SaveCampaignSchema = z.object({ id: z.string().optional(), name: z.string().min(1).max(120), subject: z.string().min(1).max(180), preview_text: z.string().max(240).optional().default(""), template_key: z.enum(["structured_campaign_v1", "spring_rotation_launch_v1", "raw_html_v1"]).optional(), content: z.array(CampaignBlockSchema).min(1).max(30), audience_definition: z.record(z.string(), z.unknown()), utm_campaign: z.string().regex(/^[a-z0-9][a-z0-9-_]{2,79}$/i).optional() })
