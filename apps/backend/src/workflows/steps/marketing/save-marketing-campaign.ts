...
export const saveMarketingCampaignStep = createStep("save-marketing-campaign", async (input: SaveCampaignInput, { container }) => {
  ...
  if (input.id) {
    ...
    const updated = await service.updateMarketingCampaigns({ id: input.id, name: input.name, subject: input.subject, preview_text: input.preview_text, template_key: input.template_key || "structured_campaign_v1", content: { blocks: input.content }, audience_definition: input.audience_definition, utm_campaign: input.utm_campaign })
  ...
  const created = await service.createMarketingCampaigns({ ...input, content: { blocks: input.content }, status: "draft", audience_snapshot_count: 0, excluded_snapshot_count: 0, sender: process.env.MUSE_EMAIL_FROM || "MUSE NZ <orders@musenz.com>", template_key: input.template_key || "structured_campaign_v1" })
