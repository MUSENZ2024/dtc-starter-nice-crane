...
export const sendMarketingEmailEventStep = createStep("send-marketing-email-event", async ({ event_id }: { event_id: string }, { container }) => {
  ...
  try {
    ...
    const notifications = await container.resolve(Modules.NOTIFICATION).createNotifications({ to: subscriber.email_normalized, from: process.env.MUSE_EMAIL_FROM || "hello@musenz.com", channel: "email", content: { subject: event.subject_snapshot, html } })
