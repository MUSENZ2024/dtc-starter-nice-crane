  ...
  for (const campaign of campaigns) {
    ...
    for (const recipient of recipients) {
      ...
      try {
        ...
        const html = await renderCampaignEmail({ subject: campaign.subject, previewText: campaign.preview_text, blocks: (campaign.content.blocks || []) as CampaignBlock[], unsubscribeUrl: `${process.env.STOREFRONT_URL || "https://musenz.com"}/marketing/unsubscribe?token=${event.tracking_token}`, utmCampaign: campaign.utm_campaign, templateKey: campaign.template_key })
