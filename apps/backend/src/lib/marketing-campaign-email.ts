...
const allowUrl = (value: unknown) => { const url = String(value || ""); return /^(https:\/\/([a-z0-9-]+\.)*musenz\.com|https?:\/\/localhost(?::\d+)?)(\/|$)/i.test(url) ? url : "https://store.musenz.com" }
...
export const renderCampaignEmail = ({ subject, previewText, blocks, unsubscribeUrl, utmCampaign }: { subject: string; previewText: string; blocks: CampaignBlock[]; unsubscribeUrl: string; utmCampaign: string }) => {
  const renderBlock = (block: CampaignBlock) => {
    ...
    if (block.type === "product_grid") return `<section><h2>${escape(block.heading || "Shop the edit")}</h2><p>${escape(block.product_handles)}</p><a href="https://store.musenz.com/store?utm_source=muse_email&utm_medium=email&utm_campaign=${encodeURIComponent(utmCampaign)}">SHOP PRODUCTS</a></section>`
