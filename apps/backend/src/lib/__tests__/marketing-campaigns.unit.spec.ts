...
describe("marketing campaign safeguards", () => {
  ...
  test("structured renderer rejects arbitrary blocks and includes unsubscribe", () => { expect(validateCampaignContent([{type:"html",body:"<script>"}])).toHaveLength(1); const html=renderCampaignEmail({subject:"Drop",previewText:"Preview",blocks:[{type:"text",heading:"Hello",body:"World"}],unsubscribeUrl:"https://musenz.com/marketing/unsubscribe",utmCampaign:"drop"}); expect(html).toContain("Unsubscribe"); expect(html).not.toContain("<script>") })
  test("Spring Rotation campaign renders the approved React email", async () => { const html=await renderCampaignEmail({subject:"Spring Rotation",previewText:"Nine spring styles, with pairs from $140.",blocks:[{type:"hero"}],unsubscribeUrl:"https://musenz.com/marketing/unsubscribe?preview=1",utmCampaign:"spring_rotation_2026",templateKey:"spring_rotation_launch_v1"}); expect(html).toContain("Find your spring pair"); expect(html).toContain("email-hero.jpg"); expect(html).toContain("marketing/unsubscribe?preview=1") })
