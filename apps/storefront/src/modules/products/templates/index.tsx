...
const ProductTemplate = async ({
  ...
  const storedWrittenReviews =
    storedReviews?.reviews.filter((review) => !review.image_url) ?? null
  const featuredWrittenReviews = storedWrittenReviews?.slice(0, 4) ?? null
  const storedPhotoReviews =
    storedReviews?.reviews.filter((review) => review.image_url) ?? null
  ...
  return (
    ...
    >
      ...
      <section className="muse-product-layout mx-auto grid min-w-0 max-w-[1440px] gap-7 px-[18px] py-4 pb-20 small:grid-cols-[1.5fr_1fr] small:gap-10 small:px-8 small:py-6 small:pb-[72px]">
        <div className="min-w-0 small:sticky small:top-28 small:self-start">
          <ImageGallery
            ...
            colourImageMap={
              (product.metadata?.colour_image_map as
      ...
      <section
        id="reviews"
        className="mx-auto max-w-[1320px] border-t border-[#E8E6E0] px-[18px] py-12 small:px-8 small:py-16"
      >
          ...
          <div className="mb-8 border-t border-[#E8E6E0] pt-8">
              ...
              {featuredWrittenReviews.map((review) => (
                ...
                >
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <span
                      className="text-[12px] tracking-[1px] text-[#C1440E]"
                  ...
                  <ExpandableReviewText
                    text={review.content}
                    collapsedLines={2}
                    className="text-[12.5px] font-medium leading-5 text-[#333]"
                  />
            ...
            <details className="mt-3 border-y border-[#E8E6E0]">
              <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-4 py-3 text-[12px] font-black uppercase tracking-[0.06em] text-[#0A0A0A] [&::-webkit-details-marker]:hidden">
                ...
                <span
                  className="text-[20px] font-normal text-[#666]"
                  aria-hidden="true"
                >
                  +
                </span>
              ...
              <div className="mt-5 grid gap-3 small:grid-cols-2">
                {textReviews.map((review) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <span
                        className="text-sm tracking-[1px] text-[#C1440E]"
                        aria-label={`${review.rating} out of 5 stars`}
                      >
                        {"★".repeat(review.rating)}
                        {"☆".repeat(5 - review.rating)}
